...
# Deploy Phishing Attack Command
@app.route('/deploy_phishing', methods=['POST'])
def deploy_phishing():
    data = request.json
    command = data.get('command')
    
    # Execute the phishing attack command
    try:
        process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        output, error = process.communicate()
        
        if error:
            return jsonify({"status": "Error occurred", "error": error.decode()})
        
        return jsonify({"status": "Phishing attack deployed successfully", "output": output.decode()})
    
    except Exception as e:
        return jsonify({"status": "Failed to execute command", "error": str(e)})

# Get network interfaces for the dropdown
@app.route('/get_interfaces', methods=['GET'])
def get_interfaces():
    interfaces = ["wlan0", "wlan1", "wlan2"]  # Example static list, dynamically populated in a real environment
